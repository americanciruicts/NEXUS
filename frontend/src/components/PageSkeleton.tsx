/**
 * Generic page loading skeleton — shown via route-level loading.tsx files
 * while a route segment streams in. Gives users an immediate, structured
 * placeholder instead of a blank white screen during slow data loads.
 */
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 dark:from-slate-900 dark:via-slate-900 dark:to-slate-800 p-4">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden">
        ...
        {Array.from({ length: rows }).map((_, i) => (
          <div key={i} className="h-12 border-b border-gray-100 dark:border-slate-700 animate-pulse" />
        ))}
      </div>
    </div>
  );
