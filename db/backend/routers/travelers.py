...
async def get_latest_revision_by_job_only(
    job_number: str,
    db: Session = Depends(get_db)
):
    """Get the latest revision traveler for a given job number (any work order)"""
    from sqlalchemy.orm import joinedload
    ...
    # Manually serialize to ensure process_steps are included
    print(f"[DEBUG] Found traveler for job {job_number} with {len(traveler.process_steps)} process steps")
    for step in traveler.process_steps:
        print(f"[DEBUG] Step {step.step_number}: work_center_code='{step.work_center_code}', operation='{step.operation}'")
    ...
    result = {
        ...
        "customer_code": traveler.customer_code,
        "customer_name": traveler.customer_name,
        "customer_revision": getattr(traveler, 'customer_revision', ''),
    ...
    print(f"Returning latest traveler for job {job_number} with {len(result['process_steps'])} process steps")
    return result
...
async def get_traveler(
    ...
    # Manually serialize to ensure process_steps are included with all fields
    result = {
        "id": traveler.id,
        "job_number": traveler.job_number,
        "work_order_number": traveler.work_order_number,
...
async def patch_traveler(
    ...
    """Partially update a traveler (e.g., toggle active status, archive)"""
    from models import TravelerStatus, Priority
        ...
        if hasattr(traveler, key):
            ...
            if key == 'status' and isinstance(value, str):
                try:
                    ...
                    print(f"Converted status string '{updates[key]}' to enum: {value}")
                except ValueError:
                ...
                try:
                    ...
                    print(f"Converted priority string '{updates[key]}' to enum: {value}")
                except ValueError:
    ...
    print(f"After commit - status: {traveler.status}, is_active: {traveler.is_active}")
